...
def synonym_replacement(sentence):
    ...
    if (synonyms):
        synonym = random.choice(synonyms).lemmas()[0].name()  # 获取词的同义词
        sentence = sentence.replace(word, synonym)
    return sentence


model_name_trans = 'D:\\AIClass_demo\\AIClass_demo0\\single_work\\opus-mt-en-de'
...
def back_translation(sentence, target_lang='de', source_lang='en'):
    # 将文本翻译为目标语言
    translated = tokenizer_trans.encode(sentence, return_tensors="pt", truncation=True, padding=True)
    translated = model_trans.generate(translated)
    translated_text = tokenizer_trans.decode(translated[0], skip_special_tokens=True)

    # 回译回来
    back_model_name = 'D:\\AIClass_demo\\AIClass_demo0\\single_work\\opus-mt-en-de'
    back_model = MarianMTModel.from_pretrained(back_model_name)
    back_tokenizer = MarianTokenizer.from_pretrained(back_model_name)

    translated_back = back_tokenizer.encode(translated_text, return_tensors="pt", truncation=True, padding=True)
    back_translated = back_model.generate(translated_back)
    back_translated_text = back_tokenizer.decode(back_translated[0], skip_special_tokens=True)
...
def augment_data(premise, hypothesis):
    if random.random() > 0.3:
        premise = back_translation(premise, target_lang='de')
        hypothesis = back_translation(hypothesis, target_lang='de')
    if random.random() > 0.4:
        premise = synonym_replacement(premise)
        hypothesis = synonym_replacement(hypothesis)
    if random.random() > 0.6:
        premise = random_deletion(premise, p=0.1)
        hypothesis = random_deletion(hypothesis, p=0.1)
...
# 使用余弦退火来衰减学习率，且学习率最低衰减到1e-7
scheduler = get_cosine_schedule_with_warmup(optimizer,
                                           # num_warmup_steps=int(0.15 * total_steps),
                                            num_training_steps=total_steps,
                                            num_cycles=0.5,  # 可以调整周期数，通常为0.5
                                            last_epoch=-1)  # 初始化从epoch 0开始
